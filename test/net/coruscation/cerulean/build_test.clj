  ...
  (testing ""
    (let [uid (.trim (:out (process/sh "id -u")))
      ...
      (test-utils/with-temp-workspace
        (process/shell (str "docker run --rm -it --user " uid ":" gid " -v ./:/workspace cerulean:latest init")
                       {:dir config/*workspace*
                        :err (logging/log-stream :error *ns*)
                        :out (logging/log-stream :info *ns*)})
        (process/shell (str "docker run --rm -it --user " uid ":" gid " -v ./:/workspace cerulean:latest build")
                       {:dir config/*workspace*
                        :err (logging/log-stream :error *ns*)
                        :out (logging/log-stream :info *ns*)})
