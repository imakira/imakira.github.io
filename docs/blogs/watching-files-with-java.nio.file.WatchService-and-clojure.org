...
The program constantly polls and prints new events in the main function, and if the newly created object is a directory and ~--recursive~ option is on. It recursively watch the newly created directory. However, it won't print files in the new directory if we check the ~inotifytools_watch_recursively~ function.
