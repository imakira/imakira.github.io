import "./cljs_env.js";
import "./shadow.js.js";
import "./module$node_modules$buffer$index.js";
import "./module$node_modules$process$browser.js";
shadow$provide.module$node_modules$axios$dist$browser$axios_cjs = function(require, module, exports, global) {
  ...
  })(new URL(platform.origin), platform.navigator && /(msie|trident)/i.test(platform.navigator.userAgent)) : () => !0, cookies = platform.hasStandardBrowserEnv ? {write(name, value, expires, path, domain, secure, sameSite) {
    typeof document !== "undefined" && (name = [`${name}=${encodeURIComponent(value)}`], utils$1.isNumber(expires) && name.push(`expires=${(new Date(expires)).toUTCString()}`), utils$1.isString(path) && name.push(`path=${path}`), utils$1.isString(domain) && name.push(`domain=${domain}`), secure === !0 && name.push("secure"), utils$1.isString(sameSite) && name.push(`SameSite=${sameSite}`), document.cookie = name.join("; "));
  }, read(name) {
    return typeof document === "undefined" ? null : (name = document.cookie.match(new RegExp("(?:^|; )" + name + "\x3d([^;]*)"))) ? decodeURIComponent(name[1]) : null;
  }, remove(name) {
  ...
  var adapters = {getAdapter:function(adapters, config) {
