shadow$provide.module$node_modules$react_dom$cjs$react_dom_server_browser_development = function(global, require, module, exports) {
  (function() {
    ...
    function writeChunk(destination, chunk) {
      if (chunk.length !== 0) {
        if (chunk.length > 512) {
          writtenBytes > 0 && (destination.enqueue(new Uint8Array(currentView.buffer, 0, writtenBytes)), currentView = new Uint8Array(512), writtenBytes = 0), destination.enqueue(chunk);
        } else {
          var allowableBytes = currentView.length - writtenBytes;
          allowableBytes < chunk.length && (allowableBytes === 0 ? destination.enqueue(currentView) : (currentView.set(chunk.subarray(0, allowableBytes), writtenBytes), destination.enqueue(currentView), chunk = chunk.subarray(allowableBytes)), currentView = new Uint8Array(512), writtenBytes = 0);
    ...
    function createResponseState(identifierPrefix, nonce, bootstrapScriptContent, bootstrapScripts, bootstrapModules) {
      ...
      nonce = nonce === void 0 ? startInlineScript : stringToPrecomputedChunk('\x3cscript nonce\x3d"' + escapeTextForBrowser(nonce) + '"\x3e');
      var bootstrapChunks = [];
      if (bootstrapScriptContent !== void 0) {
        var JSCompiler_temp_const = bootstrapChunks.push;
        checkHtmlStringCoercion(bootstrapScriptContent);
        bootstrapScriptContent = ("" + bootstrapScriptContent).replace(scriptRegex, scriptReplacer);
        JSCompiler_temp_const.call(bootstrapChunks, nonce, stringToChunk(bootstrapScriptContent), endInlineScript);
      }
      if (bootstrapScripts !== void 0) {
        for (bootstrapScriptContent = 0; bootstrapScriptContent < bootstrapScripts.length; bootstrapScriptContent++) {
          bootstrapChunks.push(startScriptSrc, stringToChunk(escapeTextForBrowser(bootstrapScripts[bootstrapScriptContent])), endAsyncScript);
        }
      }
      if (bootstrapModules !== void 0) {
        for (bootstrapScripts = 0; bootstrapScripts < bootstrapModules.length; bootstrapScripts++) {
          bootstrapChunks.push(startModuleSrc, stringToChunk(escapeTextForBrowser(bootstrapModules[bootstrapScripts])), endAsyncScript);
        }
      }
    ...
    function createRootFormatContext(namespaceURI) {
      return createFormatContext(namespaceURI === "http://www.w3.org/2000/svg" ? 2 : namespaceURI === "http://www.w3.org/1998/Math/MathML" ? 3 : 0, null);
    }
        ...
        if (hasOwnProperty.call(style, styleName)) {
          ...
          if (styleValue != null && typeof styleValue !== "boolean" && styleValue !== "") {
            if (styleName.indexOf("--") === 0) {
              ...
              chunk !== void 0 ? nameChunk = chunk : (chunk = stringToPrecomputedChunk(escapeTextForBrowser(nameChunk.replace(uppercasePattern, "-$1").toLowerCase().replace(msPattern$1, "-ms-"))), styleNameCache.set(nameChunk, chunk), nameChunk = chunk);
              typeof styleValue === "number" ? styleValue = styleValue === 0 || hasOwnProperty.call(isUnitlessNumber, styleName) ? stringToChunk("" + styleValue) : stringToChunk(styleValue + "px") : (checkCSSPropertyStringCoercion(styleValue, styleName), styleValue = stringToChunk(escapeTextForBrowser(("" + styleValue).trim())));
            }
            responseState ? (responseState = !1, target.push(styleAttributeStart, nameChunk, styleAssign, styleValue)) : target.push(styleSeparator, nameChunk, styleAssign, styleValue);
          }
        }
      }
    ...
    function pushAttribute(target, responseState, name, value) {
      ...
      if (!(name.length > 2) || name[0] !== "o" && name[0] !== "O" || name[1] !== "n" && name[1] !== "N") {
        if (responseState = getPropertyInfo(name), responseState !== null) {
          ...
          var attributeName = responseState.attributeName;
          name = stringToChunk(attributeName);
          switch(responseState.type) {
            case 3:
              value && target.push(attributeSeparator, name, attributeEmptyString);
              break;
            case 4:
              value === !0 ? target.push(attributeSeparator, name, attributeEmptyString) : value !== !1 && target.push(attributeSeparator, name, attributeAssign, stringToChunk(escapeTextForBrowser(value)), attributeEnd);
              break;
            case 5:
              isNaN(value) || target.push(attributeSeparator, name, attributeAssign, stringToChunk(escapeTextForBrowser(value)), attributeEnd);
              break;
            case 6:
              !isNaN(value) && value >= 1 && target.push(attributeSeparator, name, attributeAssign, stringToChunk(escapeTextForBrowser(value)), attributeEnd);
              break;
            default:
              responseState.sanitizeURL && (checkAttributeStringCoercion(value, attributeName), responseState = value = "" + value, !didWarn && isJavaScriptProtocol.test(responseState) && (didWarn = !0, error("A future version of React will block javascript: URLs as a security precaution. Use event handlers instead if you can. If you need to generate unsafe HTML try using dangerouslySetInnerHTML instead. React was passed %s.", JSON.stringify(responseState)))), target.push(attributeSeparator, name, 
              attributeAssign, stringToChunk(escapeTextForBrowser(value)), attributeEnd);
          }
    ...
    function startChunkForTag(tag) {
      ...
      if (tagStartChunk === void 0) {
        ...
        tagStartChunk = stringToPrecomputedChunk("\x3c" + tag);
      ...
      switch(type) {
        ...
        case "option":
          ...
          if (innerHTML != null) {
            if (value !== null ? (checkAttributeStringCoercion(value, "value"), props = "" + value) : (propKey === null || didWarnInvalidOptionInnerHTML || (didWarnInvalidOptionInnerHTML = !0, error("Pass a `value` prop if you set dangerouslyInnerHTML so React knows which value should be selected.")), props = flattenOptionChildren(propValue)), isArrayImpl(innerHTML)) {
              ...
              checkAttributeStringCoercion(innerHTML, "select.value"), "" + innerHTML === props && target.push(selectedMarkerAttribute);
            }
          } else {
        ...
        case "pre":
          ...
          if (innerHTML != null) {
            ...
            props !== null && props !== void 0 && (typeof props === "string" && props.length > 0 && props[0] === "\n" ? target.push(leadingNewline, stringToChunk(props)) : (checkHtmlStringCoercion(props), target.push(stringToChunk("" + props))));
          }
          typeof children === "string" && children[0] === "\n" && target.push(leadingNewline);
          return children;
        case "area":
        ...
        case "html":
          return formatContext.insertionMode === 0 && target.push(DOCTYPE), pushStartGenericElement(target, props, type, responseState);
        default:
            ...
            if (hasOwnProperty.call(props, selected) && (propValue = props[selected], propValue != null)) {
              switch(selected) {
                ...
                case "suppressHydrationWarning":
                  break;
                default:
                  isAttributeNameSafe(selected) && typeof propValue !== "function" && typeof propValue !== "symbol" && target.push(attributeSeparator, stringToChunk(selected), attributeAssign, stringToChunk(escapeTextForBrowser(propValue)), attributeEnd);
              }
            }
          }
          target.push(endOfStartTag);
    ...
    function mountClassInstance(instance, ctor, newProps, maskedLegacyContext) {
      ...
      instance.context = typeof contextType === "object" && contextType !== null ? contextType._currentValue : maskedLegacyContext;
    ...
    function fatalError(request, error) {
      ...
      onShellError(error);
      request.destination !== null ? (request.status = 2, closeWithError(request.destination, error)) : (request.status = 1, request.fatalError = error);
    }
    ...
    function renderElement(request, task, type, props, ref) {
      if (typeof type === "function") {
        if (type.prototype && type.prototype.isReactComponent) {
          ...
          contextType = new type(props, typeof contextType === "object" && contextType !== null ? contextType._currentValue : ref);
        ...
        if (typeof type === "string") {
          ...
          switch(type) {
            ...
            case "wbr":
              break;
            default:
              ref.chunks.push(endTag1, stringToChunk(type), endTag2);
          }
          ref.lastPushedText = !1;
          popComponentStackInDEV(task);
        } else {
          switch(type) {
            ...
            case REACT_SUSPENSE_TYPE:
              a: {
                ...
                try {
                  if (renderNode(request, task, props), _componentName.lastPushedText && _componentName.textEmbedded && _componentName.chunks.push(textSeparator), _componentName.status = 1, queueCompletedSegment(foundWillReceivePropsName, _componentName), foundWillReceivePropsName.pendingTasks === 0) {
                    popComponentStackInDEV(task);
                    break a;
                  }
                } catch (error) {
          ...
          if (typeof type === "object" && type !== null) {
            switch(type.$$typeof) {
              ...
              case REACT_PROVIDER_TYPE:
                ...
                props = task.context;
                addendum = type._currentValue;
                type._currentValue = ref;
                type._currentRenderer !== void 0 && type._currentRenderer !== null && type._currentRenderer !== rendererSigil && error("Detected multiple renderers concurrently rendering the same context provider. This is currently unsupported.");
                ...
                request.context._currentValue = ref === REACT_SERVER_CONTEXT_DEFAULT_VALUE_NOT_LOADED ? request.context._defaultValue : ref;
                type._currentRenderer !== void 0 && type._currentRenderer !== null && type._currentRenderer !== rendererSigil && error("Detected multiple renderers concurrently rendering the same context provider. This is currently unsupported.");
                type._currentRenderer = rendererSigil;
    ...
    function abortTask(task, request, reason) {
      ...
      if (boundary === null) {
        request.allPendingTasks--, request.status !== 2 && (request.status = 2, request.destination !== null && request.destination.close());
      } else {
        ...
        try {
          ...
          for (i = 0; i < pingedTasks.length; i++) {
            ...
            if (segment.status === 0) {
              ...
              currentTaskInDEV = task;
              try {
                renderNodeDestructive(request, task, task.node), segment.lastPushedText && segment.textEmbedded && segment.chunks.push(textSeparator), task.abortSet.delete(task), segment.status = 1, finishedTask(request, task.blockedBoundary, segment);
              } catch (x) {
                if (resetHooksState(), typeof x === "object" && x !== null && typeof x.then === "function") {
                  var ping = task.ping;
                  x.then(ping, ping);
    ...
    function flushSubtree(request, destination, segment) {
      ...
      switch(segment.status) {
        ...
        case 1:
          ...
          chunkIdx < segmentID.length && (r = writeChunkAndReturn(destination, segmentID[chunkIdx]));
          return r;
        default:
    ...
    function flushSegment(request, destination, segment) {
      ...
      if (boundary.forceClientRender) {
        ...
        errorMesssage && (writeChunk(destination, clientRenderedSuspenseBoundaryError1B), writeChunk(destination, stringToChunk(escapeTextForBrowser(errorMesssage))), writeChunk(destination, clientRenderedSuspenseBoundaryErrorAttrInterstitial));
        boundary && (writeChunk(destination, clientRenderedSuspenseBoundaryError1C), writeChunk(destination, stringToChunk(escapeTextForBrowser(boundary))), writeChunk(destination, clientRenderedSuspenseBoundaryErrorAttrInterstitial));
        writeChunkAndReturn(destination, clientRenderedSuspenseBoundaryError2);
        flushSubtree(request, destination, segment);
      } else if (boundary.pendingTasks > 0) {
        boundary.rootSegmentID = request.nextSegmentId++, boundary.completedSegments.length > 0 && request.partialBoundaries.push(boundary), errorDigest = request.responseState, errorMesssage = errorDigest.nextSuspenseID++, errorDigest = stringToPrecomputedChunk(errorDigest.boundaryPrefix + errorMesssage.toString(16)), boundary = boundary.id = errorDigest, writeStartPendingSuspenseBoundary(destination, request.responseState, boundary), flushSubtree(request, destination, segment);
      } else if (boundary.byteSize > request.progressiveChunkSize) {
        boundary.rootSegmentID = request.nextSegmentId++, request.completedBoundaries.push(boundary), writeStartPendingSuspenseBoundary(destination, request.responseState, boundary.id), flushSubtree(request, destination, segment);
      } else {
    ...
    function flushCompletedBoundary(request, destination, boundary) {
      ...
      request.sentCompleteBoundaryFunction ? writeChunk(destination, completeBoundaryScript1Partial) : (request.sentCompleteBoundaryFunction = !0, writeChunk(destination, completeBoundaryScript1Full));
    ...
    function flushPartiallyCompletedSegment(request, destination, boundary, segment) {
      ...
      request = request.responseState;
      writeChunk(destination, request.startInlineScript);
      request.sentCompleteSegmentFunction ? writeChunk(destination, completeSegmentScript1Partial) : (request.sentCompleteSegmentFunction = !0, writeChunk(destination, completeSegmentScript1Full));
      writeChunk(destination, request.segmentPrefix);
    ...
    function flushCompletedQueues(request, destination) {
      ...
      try {
        ...
        if (completedRootSegment !== null && request.pendingRootTasks === 0) {
          ...
          completedRootSegment < bootstrapChunks.length && writeChunkAndReturn(destination, bootstrapChunks[completedRootSegment]);
        }
        ...
        for (i = 0; i < clientRenderedBoundaries.length; i++) {
          ...
          responseState.sentClientRenderFunction ? writeChunk(bootstrapChunks, clientRenderScript1Partial) : (responseState.sentClientRenderFunction = !0, writeChunk(bootstrapChunks, clientRenderScript1Full));
          if (boundaryID === null) {
            throw Error("An ID must have been assigned before we can complete the boundary.");
          }
          writeChunk(bootstrapChunks, boundaryID);
          writeChunk(bootstrapChunks, clientRenderScript1A);
          if (errorDigest || errorMessage || errorComponentStack) {
            writeChunk(bootstrapChunks, clientRenderErrorScriptArgInterstitial), writeChunk(bootstrapChunks, stringToChunk(escapeJSStringsForInstructionScripts(errorDigest || "")));
          }
          if (errorMessage || errorComponentStack) {
            writeChunk(bootstrapChunks, clientRenderErrorScriptArgInterstitial), writeChunk(bootstrapChunks, stringToChunk(escapeJSStringsForInstructionScripts(errorMessage || "")));
          }
          errorComponentStack && (writeChunk(bootstrapChunks, clientRenderErrorScriptArgInterstitial), writeChunk(bootstrapChunks, stringToChunk(escapeJSStringsForInstructionScripts(errorComponentStack))));
          if (!writeChunkAndReturn(bootstrapChunks, clientRenderScript2)) {
        ...
        completeWriting(destination), request.allPendingTasks === 0 && request.pingedTasks.length === 0 && request.clientRenderedBoundaries.length === 0 && request.completedBoundaries.length === 0 && (request.abortableTasks.size !== 0 && error("There was still abortable task at the root when we closed. This is a bug in React."), destination.close());
      }
    }
    ...
    }, matchHtmlRegExp = /["'&<>]/, uppercasePattern = /([A-Z])/g, msPattern$1 = /^ms-/, isJavaScriptProtocol = /^[\u0000-\u001F ]*j[\r\n\t]*a[\r\n\t]*v[\r\n\t]*a[\r\n\t]*s[\r\n\t]*c[\r\n\t]*r[\r\n\t]*i[\r\n\t]*p[\r\n\t]*t[\r\n\t]*:/i, didWarn = !1, isArrayImpl = Array.isArray, startInlineScript = stringToPrecomputedChunk("\x3cscript\x3e"), endInlineScript = stringToPrecomputedChunk("\x3c/script\x3e"), startScriptSrc = stringToPrecomputedChunk('\x3cscript src\x3d"'), startModuleSrc = stringToPrecomputedChunk('\x3cscript type\x3d"module" src\x3d"'), 
    endAsyncScript = stringToPrecomputedChunk('" async\x3d""\x3e\x3c/script\x3e'), scriptRegex = /(<\/|<)(s)(cript)/gi, scriptReplacer = function(match, prefix, s, suffix) {
      return "" + prefix + (s === "s" ? "\\u0073" : "\\u0053") + suffix;
    }, textSeparator = stringToPrecomputedChunk("\x3c!-- --\x3e"), styleNameCache = new Map(), styleAttributeStart = stringToPrecomputedChunk(' style\x3d"'), styleAssign = stringToPrecomputedChunk(":"), styleSeparator = stringToPrecomputedChunk(";"), attributeSeparator = stringToPrecomputedChunk(" "), attributeAssign = stringToPrecomputedChunk('\x3d"'), attributeEnd = stringToPrecomputedChunk('"'), attributeEmptyString = stringToPrecomputedChunk('\x3d""'), endOfStartTag = stringToPrecomputedChunk("\x3e"), 
    endOfStartTagSelfClosing = stringToPrecomputedChunk("/\x3e"), didWarnDefaultInputValue = !1, didWarnDefaultChecked = !1, didWarnDefaultSelectValue = !1, didWarnDefaultTextareaValue = !1, didWarnInvalidOptionChildren = !1, didWarnInvalidOptionInnerHTML = !1, didWarnSelectedSetOnOption = !1, selectedMarkerAttribute = stringToPrecomputedChunk(' selected\x3d""'), leadingNewline = stringToPrecomputedChunk("\n"), VALID_TAG_REGEX = /^[a-zA-Z][a-zA-Z:_\.\-\d]*$/, validatedTagCache = new Map(), DOCTYPE = 
    stringToPrecomputedChunk("\x3c!DOCTYPE html\x3e"), endTag1 = stringToPrecomputedChunk("\x3c/"), endTag2 = stringToPrecomputedChunk("\x3e"), placeholder1 = stringToPrecomputedChunk('\x3ctemplate id\x3d"'), placeholder2 = stringToPrecomputedChunk('"\x3e\x3c/template\x3e'), startCompletedSuspenseBoundary = stringToPrecomputedChunk("\x3c!--$--\x3e"), startPendingSuspenseBoundary1 = stringToPrecomputedChunk('\x3c!--$?--\x3e\x3ctemplate id\x3d"'), startPendingSuspenseBoundary2 = stringToPrecomputedChunk('"\x3e\x3c/template\x3e'), 
    startClientRenderedSuspenseBoundary = stringToPrecomputedChunk("\x3c!--$!--\x3e"), endSuspenseBoundary = stringToPrecomputedChunk("\x3c!--/$--\x3e"), clientRenderedSuspenseBoundaryError1 = stringToPrecomputedChunk("\x3ctemplate"), clientRenderedSuspenseBoundaryErrorAttrInterstitial = stringToPrecomputedChunk('"'), clientRenderedSuspenseBoundaryError1A = stringToPrecomputedChunk(' data-dgst\x3d"'), clientRenderedSuspenseBoundaryError1B = stringToPrecomputedChunk(' data-msg\x3d"'), clientRenderedSuspenseBoundaryError1C = 
    stringToPrecomputedChunk(' data-stck\x3d"'), clientRenderedSuspenseBoundaryError2 = stringToPrecomputedChunk("\x3e\x3c/template\x3e"), startSegmentHTML = stringToPrecomputedChunk('\x3cdiv hidden id\x3d"'), startSegmentHTML2 = stringToPrecomputedChunk('"\x3e'), endSegmentHTML = stringToPrecomputedChunk("\x3c/div\x3e"), startSegmentSVG = stringToPrecomputedChunk('\x3csvg aria-hidden\x3d"true" style\x3d"display:none" id\x3d"'), startSegmentSVG2 = stringToPrecomputedChunk('"\x3e'), endSegmentSVG = 
    stringToPrecomputedChunk("\x3c/svg\x3e"), startSegmentMathML = stringToPrecomputedChunk('\x3cmath aria-hidden\x3d"true" style\x3d"display:none" id\x3d"'), startSegmentMathML2 = stringToPrecomputedChunk('"\x3e'), endSegmentMathML = stringToPrecomputedChunk("\x3c/math\x3e"), startSegmentTable = stringToPrecomputedChunk('\x3ctable hidden id\x3d"'), startSegmentTable2 = stringToPrecomputedChunk('"\x3e'), endSegmentTable = stringToPrecomputedChunk("\x3c/table\x3e"), startSegmentTableBody = stringToPrecomputedChunk('\x3ctable hidden\x3e\x3ctbody id\x3d"'), 
    startSegmentTableBody2 = stringToPrecomputedChunk('"\x3e'), endSegmentTableBody = stringToPrecomputedChunk("\x3c/tbody\x3e\x3c/table\x3e"), startSegmentTableRow = stringToPrecomputedChunk('\x3ctable hidden\x3e\x3ctr id\x3d"'), startSegmentTableRow2 = stringToPrecomputedChunk('"\x3e'), endSegmentTableRow = stringToPrecomputedChunk("\x3c/tr\x3e\x3c/table\x3e"), startSegmentColGroup = stringToPrecomputedChunk('\x3ctable hidden\x3e\x3ccolgroup id\x3d"'), startSegmentColGroup2 = stringToPrecomputedChunk('"\x3e'), 
    endSegmentColGroup = stringToPrecomputedChunk("\x3c/colgroup\x3e\x3c/table\x3e"), completeSegmentScript1Full = stringToPrecomputedChunk('function $RS(a,b){a\x3ddocument.getElementById(a);b\x3ddocument.getElementById(b);for(a.parentNode.removeChild(a);a.firstChild;)b.parentNode.insertBefore(a.firstChild,b);b.parentNode.removeChild(b)};$RS("'), completeSegmentScript1Partial = stringToPrecomputedChunk('$RS("'), completeSegmentScript2 = stringToPrecomputedChunk('","'), completeSegmentScript3 = stringToPrecomputedChunk('")\x3c/script\x3e'), 
    completeBoundaryScript1Full = stringToPrecomputedChunk('function $RC(a,b){a\x3ddocument.getElementById(a);b\x3ddocument.getElementById(b);b.parentNode.removeChild(b);if(a){a\x3da.previousSibling;var f\x3da.parentNode,c\x3da.nextSibling,e\x3d0;do{if(c\x26\x268\x3d\x3d\x3dc.nodeType){var d\x3dc.data;if("/$"\x3d\x3d\x3dd)if(0\x3d\x3d\x3de)break;else e--;else"$"!\x3d\x3dd\x26\x26"$?"!\x3d\x3dd\x26\x26"$!"!\x3d\x3dd||e++}d\x3dc.nextSibling;f.removeChild(c);c\x3dd}while(c);for(;b.firstChild;)f.insertBefore(b.firstChild,c);a.data\x3d"$";a._reactRetry\x26\x26a._reactRetry()}};$RC("'), 
    ...
    REACT_LEGACY_HIDDEN_TYPE = Symbol.for("react.legacy_hidden"), REACT_SERVER_CONTEXT_DEFAULT_VALUE_NOT_LOADED = Symbol.for("react.default_value"), MAYBE_ITERATOR_SYMBOL = Symbol.iterator, disabledDepth = 0, prevLog, prevInfo, prevWarn, prevError, prevGroup, prevGroupCollapsed, prevGroupEnd;
    ...
    exports.renderToReadableStream = function(children, options) {
      return new Promise(function(resolve, reject) {
        var onFatalError, onAllReady, allReady = new Promise(function(res, rej) {
          onAllReady = res;
          onFatalError = rej;
        }), request$jscomp$0 = createRequest(children, createResponseState(options ? options.identifierPrefix : void 0, options ? options.nonce : void 0, options ? options.bootstrapScriptContent : void 0, options ? options.bootstrapScripts : void 0, options ? options.bootstrapModules : void 0), createRootFormatContext(options ? options.namespaceURI : void 0), options ? options.progressiveChunkSize : void 0, options ? options.onError : void 0, onAllReady, function() {
          var stream = new ReadableStream({type:"bytes", pull:function(controller) {
